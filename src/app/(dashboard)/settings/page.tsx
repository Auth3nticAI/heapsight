...
export default function SettingsPage() {
  ...
  const [githubRepos, setGithubRepos] = useState<Record<string, { full_name: string }>>({});
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      const { data: profileData } = await supabase
        .from("profiles")
        .select("total_xp, tier, selected_game_template, referral_code, pro_trial_until, github_username, github_repos")
        .eq("id", user.id)
        .single();
      ...
      setGithubRepos(profileData?.github_repos || {});
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6 min-w-0">
            ...
            {/* GitHub Auto-Commit */}
            <section id="github" className="p-5 rounded-2xl border border-white/[0.08] bg-[#071528]">
              <h2 className="text-sm font-semibold text-white mb-1">GitHub Auto-Commit</h2>
              <p className="text-[10px] font-mono text-[#AFBCD5]/50 mb-4">
                Automatically save your lesson code to GitHub when you complete each lesson.
              </p>

              {githubUsername ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-[#040B10] rounded-xl border border-white/[0.05]">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white font-medium truncate">@{githubUsername}</p>
                      <p className="text-[10px] font-mono text-[#AFBCD5]/50">
                        {Object.keys(githubRepos).length} repo{Object.keys(githubRepos).length !== 1 ? "s" : ""} connected
                      </p>
                    </div>
                    <CheckCircleIcon className="h-4 w-4 text-[#9CD323] shrink-0" />
                  </div>

                  {Object.entries(githubRepos).map(([repoPath, repo]) => (
                    <a
                      key={repoPath}
                      href={`https://github.com/${repo.full_name}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block text-[10px] font-mono text-primary hover:underline"
                    >
                      {repo.full_name} ({repoPath}) &rarr;
                    </a>
                  ))}
                  ...
                  <button
                    ...
                    className="text-[10px] font-mono text-red-400/70 hover:text-red-400 transition-colors"
                  >
                ...
                <a
                  href="/api/github/connect?redirect=/settings"
                  className="flex items-center justify-center gap-2 w-full py-3 bg-[#24292f] hover:bg-[#24292f]/80 text-white font-semibold text-sm rounded-xl transition-all min-h-[44px] touch-manipulation"
                >
