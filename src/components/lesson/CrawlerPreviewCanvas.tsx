...
import type { CrawlerFrame, CrawlerSensor } from "@/types/crawler";
...
function Scene({ frame }: { frame: CrawlerFrame | null }) {
  return (
    <>
      ...
      {frame ? (
        ...
        <Html center>
          <p className="text-[#444] text-xs font-mono whitespace-nowrap">
            Run your code to see the crawler
          </p>
        </Html>
      )}
      <OrbitControls
...
export default function CrawlerPreviewCanvas() {
  const crawlerFrames = useLessonStore((s) => s.crawlerFrames);
  ...
  // Step through frames
  useEffect(() => {
    if (!isAnimating || crawlerFrames.length === 0 || frameIndex < 0) return;

    if (frameIndex < crawlerFrames.length - 1) {
      ...
      setIsAnimating(false);
      const lastFrame = crawlerFrames[crawlerFrames.length - 1];
  ...
  }, [frameIndex, isAnimating, crawlerFrames]);

  const currentFrame = frameIndex >= 0 && frameIndex < crawlerFrames.length
    ? crawlerFrames[frameIndex]
    : null;

  const handleReplay = () => {
    if (crawlerFrames.length === 0) return;
    setShowOverlay(false);
    setFrameIndex(0);
    setIsAnimating(true);
  };
  ...
  const handleTogglePause = () => {
    ...
    if (isAnimating) {
      setIsAnimating(false);
    } else if (frameIndex < crawlerFrames.length - 1) {
      setIsAnimating(true);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col rounded-lg border border-[#2e2e42] bg-surface overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-1.5 bg-[#16162e] border-b border-[#2e2e42]">
        ...
        {crawlerFrames.length > 1 && (
          <span className="text-[10px] font-mono text-[#555] ml-auto">
            {frameIndex + 1}/{crawlerFrames.length}
          </span>
        )}
      </div>
      <div className="flex-1 relative" aria-label="3D crawler simulation" role="img">
        ...
        {currentFrame?.message && !showOverlay && (
          <div className="absolute bottom-2 left-2 right-2 bg-[#16162e]/90 border border-[#2e2e42] rounded px-2 py-1">
        ...
        {crawlerFrames.length > 1 && (
          <div className="absolute bottom-2 right-2 flex gap-1 z-10">
            <button
              ...
              className="bg-[#16162e]/80 border border-[#2e2e42] px-2 py-0.5 rounded text-[10px] font-mono text-[#888] hover:text-white transition-colors"
            >
              {isAnimating ? "\u23F8" : "\u25B6"}
            </button>
            <button
