...
export const lessonRPG11: Lesson = {
  id: "rpg-11-god-object-refactor",
  title: "God Object Refactor",
  description: "Refactor scattered globals into a single struct World. The game behavior stays identical -- only the data layout changes. This is the first step toward serialization and snapshots.",
  order: 11,
  xpReward: 100,
  tier: "pro",
  concepts: ["struct", "aggregate state", "refactoring", "god object", "data layout"],
  part1: {
    title: "Concept: God Object Refactor",
    type: "concept",
...
The fix: put all mutable game state in one \`struct World\`. The \`struct\` becomes the "god object" -- a single aggregate that holds everything.
\`\`\`cpp
struct World {
...
## Why "God Object"?
In OOP, a "God Object" is an anti-pattern: one class that does everything. But our \`struct World\` is pure data with no methods. The phase functions ARE the methods, kept separate by design. A data-only god object is the correct first step toward serialization.
...
## Mental Model
Your L10 micro-dungeon works perfectly -- but its state is scattered across a dozen globals. Time to refactor. You will move all mutable entity state into a \`struct World\` and prefix every access with \`world.\`. **The gameplay does not change.** This is a pure data reorganization.

## What You Have
The starter code below is your L10 solution verbatim -- bare globals everywhere. Your job: extract them into a struct.

## Step 1: Define the struct
After the intent constants and \`pending_intent\`, add:
\`\`\`cpp
struct World {
    int player_x = 5, player_y = 5;
    int enemy_x = 9, enemy_y = 7;
...
## Step 2: Delete the old globals
Remove these lines (they now live inside the struct):
- \`int player_x = 5, player_y = 5;\`
- \`int enemy_x = 9, enemy_y = 7;\`
- \`bool enemy_alive = true;\`
- \`int player_hp = 20, player_max_hp = 20;\`
...
## Step 3: Add world. prefix everywhere
In every function body, replace bare variable names with \`world.\` prefixed versions:
...
## Step 4: Add the struct confirmation line
After the existing cout block in main(), add:
\`\`\`cpp
...
// TODO 1: Define struct World { ... }; with all mutable entity state
// Move player_x/y, enemy_x/y, enemy_alive, player_hp/max_hp,
// enemy_hp/max_hp, turn_count, kills into the struct.
// Use default member initializers (e.g. int player_x = 5).
// Then declare: World world;
// Then DELETE the old bare globals listed above.
...
// TODO 2: Replace all bare player_x/player_y/enemy_x/enemy_y with world. prefix
void resolveCommand() {
...
// TODO 3: Replace bare enemy_hp/enemy_max_hp with world. prefix
void resolveCombat() {
...
// TODO 4: Replace bare enemy/player vars with world. prefix
void phaseWorld() {
...
// TODO 5: Replace bare enemy_alive/enemy_hp/kills/turn_count with world. prefix
void phaseCleanup() {
...
int main() {
    ...
    // TODO 6: Replace bare vars in cout lines with world. prefix
    ...
    // TODO 7: Add cout << "Struct: world" << endl;
    ...
    while (!WindowShouldClose()) {
        ...
        // TODO 8: Replace bare vars in DrawRectangle/DrawText with world. prefix
...
void phaseWorld() {
    ...
    if (nx >= 0 && nx < GRID_W && ny >= 0 && ny < GRID_H &&
        tiles[ny][nx] != 1 &&
        !(nx == world.player_x && ny == world.player_y)) {
        world.enemy_x = nx;
        world.enemy_y = ny;
    }
}
...
int main() {
    ...
    hints: [
      "Define struct World { int player_x = 5; ... }; World world; after the intent constants.",
      "Delete the old bare globals (player_x, enemy_x, etc.) -- they now live inside the struct.",
      "Replace every bare player_x with world.player_x, enemy_hp with world.enemy_hp, etc. in ALL functions and main().",
    ],
    estimatedMinutes: 15
  }
};