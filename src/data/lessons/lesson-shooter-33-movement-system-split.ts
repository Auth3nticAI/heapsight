...
export const lessonShooter33: Lesson = {
  id: "shooter-33-movement-system-split",
  ...
  part1: {
    title: "Concept: Movement System Split",
    type: "concept",
    instructions: `# Movement System Split

## movementSystem: One Job
Every game tick, two things move: bullets travel up, enemies travel down. That's it. Extracting this logic into movementSystem() makes it trivially testable.

\`\`\`cpp
void movementSystem(World& w) {
    for (int i=0;i<MAX_BULLETS;i++) {
        if (w.bullet_active[i]) { w.bullet_y[i]-=8; ... }
    }
    for (int i=0;i<MAX_ENEMIES;i++) {
        if (w.enemy_active[i]) { w.enemy_y[i]+=w.enemy_speed[i]*dt; ... }
    }
}
\`\`\`
...
void listSystems() {
    ...
    cout << "Pattern: movement-module" << endl;
}
int main() {
    // TODO: register input(0), movement(1), render(2)
    listSystems();
    return 0;
}`,
...
int main() {
    ...
    registerSystem(1, "movement");
    registerSystem(2, "render");
    listSystems();
    return 0;
}`,
    tests: [
      { id: "t1", description: "prints system count", expectedOutput: "Systems: 3" },
      { id: "t2", description: "prints movement system", expectedOutput: "System[1]: movement" },
      { id: "t3", description: "prints pattern", expectedOutput: "Pattern: movement-module" },
    ],
    hints: [
      "Register input(0), movement(1), render(2).",
      "listSystems() loop already works — just call registerSystem 3 times.",
    ],
    estimatedMinutes: 10,
  },
  part2: {
    title: "Build: Movement System Split",
    type: "game_builder",
    instructions: `# Build: Movement System Split

## What's Already Here
L32 code with inputSystem and renderSystem working. Bullet and enemy movement is still inlined in the physics loop.

## Your Task

**TODO 1** — Implement movementSystem(World& w):

\`\`\`cpp
void movementSystem(World& w) {
    // move all active bullets up by 8px, despawn if off-screen
    // move all active enemies down by speed*FIXED_DT, wrap at SCREEN_H
}
\`\`\`

**TODO 2** — In the fixed-timestep loop, replace the inline movement block with:
\`\`\`cpp
movementSystem(world);
\`\`\`

## Did It Work?
Physics loop now has 3 clean lines: inputSystem, movementSystem, [inline collision]. HUD shows **SYS: 3 | input move render**.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
// TODO 1: implement movementSystem
// Move bullets: bullet_y[i] -= 8; despawn if < -10
// Move enemies: enemy_y[i] += enemy_speed[i]*FIXED_DT; wrap at SCREEN_H
void movementSystem(World& w) {
...
void renderSystem(World& w) {
    ...
    DrawText("SYS: 3 | input move render",10,194,14,LIME);
    EndDrawing();
}
int main() {
    ...
    cout << "System[2]: render" << endl;
    cout << "Pattern: movement-module" << endl;
    while (!WindowShouldClose()) {
        float dt = GetFrameTime(); accumulator += dt;
        while (accumulator >= FIXED_DT) {
            inputSystem(world);
            // TODO 2: call movementSystem(world); here
            // (remove the inline movement block below)
            for (int i=0;i<MAX_BULLETS;i++) {
...
int main() {
    ...
    tests: [
      { id: "g1", description: "prints 3 systems", expectedOutput: "Systems: 3" },
      { id: "g2", description: "prints movement", expectedOutput: "System[1]: movement" },
      { id: "g3", description: "prints pattern", expectedOutput: "Pattern: movement-module" },
      { id: "g4", description: "gate A passes", expectedOutput: "GATE A: PASSED" },
    ],
    hints: [
      "TODO 1: bullet loop: bullet_y[i]-=8; if(bullet_y[i]<-10) despawnBullet(w,i);",
      "TODO 1: enemy loop: enemy_y[i]+=enemy_speed[i]*FIXED_DT; if(enemy_y[i]>SCREEN_H) enemy_y[i]=0;",
      "TODO 2: replace the 8-line movement block with movementSystem(world);",
    ],
    estimatedMinutes: 20,
  },
};
