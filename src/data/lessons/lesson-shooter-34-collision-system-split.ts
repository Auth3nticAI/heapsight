...
export const lessonShooter34: Lesson = {
  id: "shooter-34-collision-system-split",
  ...
  part1: {
    title: "Concept: Collision System Split",
    type: "concept",
    instructions: `# Collision System Split

## collisionSystem: The Referee
Collision is the most complex logic — bullets hit enemies, powerups hit ship, waves respawn. One function owns all of it.
...
int main() {
    // TODO: register input(0), movement(1), collision(2), render(3)
    listSystems();
    return 0;
}`,
...
int main() {
    ...
    registerSystem(2, "collision");
    registerSystem(3, "render");
    listSystems();
    return 0;
}`,
    tests: [
      { id: "t1", description: "prints 4 systems", expectedOutput: "Systems: 4" },
      { id: "t2", description: "prints collision system", expectedOutput: "System[2]: collision" },
  ...
  part2: {
    title: "Build: Collision System Split",
    type: "game_builder",
    instructions: `# Build: Collision System Split

## What's Already Here
L33 with inputSystem, movementSystem, renderSystem working. The inline collision block is still in the physics loop.

## Your Task

**TODO 1** — Implement collisionSystem(World& w). It should handle:
- Bullet vs enemy AABB collision (spawn particles, despawn both, +100 score, maybe spawn powerup)
- Powerup vs ship overlap (collectPowerup)
- updateParticles and updatePowerups
- Wave check: if all enemies dead, increment wave and spawnWave

**TODO 2** — Replace the inline collision block in the physics loop with collisionSystem(world);

## Did It Work?
Physics loop is now 3 clean calls: inputSystem, movementSystem, collisionSystem. HUD shows **SYS: 4 | input move col render**.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
// TODO 1: implement collisionSystem
// Handle: bullet-enemy AABB, powerup-ship overlap, updateParticles, updatePowerups, wave check
void collisionSystem(World& w) {
...
void renderSystem(World& w) {
    ...
    DrawText("SYS: 4 | input move col render",10,194,14,LIME);
    EndDrawing();
}
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        while (accumulator >= FIXED_DT) {
            ...
            // TODO 2: call collisionSystem(world); (remove inline block below)
...
int main() {
    ...
    tests: [
      ...
      { id: "g2", description: "collision system listed", expectedOutput: "System[2]: collision" },
      { id: "g3", description: "prints pattern", expectedOutput: "Pattern: collision-module" },
      { id: "g4", description: "gate A passes", expectedOutput: "GATE A: PASSED" },
    ],
    hints: [
      "The collisionSystem body is the entire INLINE_COLLISION block, changing world. to w.",
      "Bullet-enemy: nested loops, check AABB, despawnBullet+despawnEnemy, spawnParticles, score+=100.",
      "At the end: updateParticles(w); updatePowerups(w); wave check with int alive=0 counter.",
    ],
    estimatedMinutes: 25,
  },
};
