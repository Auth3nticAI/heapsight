...
export const lessonShooter41: Lesson = {
  id: "shooter-41-wave-table-parse",
  ...
  part1: {
    title: "Concept: Wave Table Parse",
    type: "concept",
    instructions: `# Wave Table Parse

## Data-Driven Waves
Instead of hardcoding wave difficulty, define a WaveConfig table. Waves loop through it with modular indexing.

\`\`\`cpp
struct WaveConfig { int enemy_count; float base_speed; };
WaveConfig wave_table[5] = {{3,18},{4,22},{5,28},{5,34},{5,40}};
WaveConfig getWaveConfig(int wave) {
    return wave_table[(wave-1)%5];
}
\`\`\`
...
const int N=5;
WaveConfig tbl[N]={{3,18},{4,22},{5,28},{5,34},{5,40}};
WaveConfig getWave(int w){return tbl[(w-1)%N];}
int main(){
    cout<<"WaveTable: "<<N<<endl;
    WaveConfig c=getWave(1);
    // TODO: print Wave 1: N enemies speed S
    cout<<"Pattern: wave-table"<<endl;
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "wave table size", expectedOutput: "WaveTable: 5" },
      { id: "t2", description: "wave 1 data", expectedOutput: "Wave 1: 3 enemies speed 18" },
  ...
  part2: {
    title: "Build: Wave Table Parse",
    type: "game_builder",
    instructions: `# Build: Wave Table Parse

## Your Task
**TODO 1** — Implement getWaveConfig(int wave) to return wave_table[(wave-1)%WAVE_TABLE_SIZE]. The current stub always returns wave_table[0].

## Did It Work?
Console: WaveTable: 5, Pattern: wave-table. Game uses wave table for difficulty scaling.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
// TODO 1: implement getWaveConfig(int wave)
// Return wave_table[(wave-1) % WAVE_TABLE_SIZE]
WaveConfig getWaveConfig(int wave) {
    // use modular index to loop through table
    return wave_table[0]; // fix this
}
void hudSystem(World& w){
    DrawText("** WAVE TABLE ACTIVE **",SCREEN_W/2-180,8,18,CYAN);
...
int main(){
    InitWindow(SCREEN_W,SCREEN_H,"HeapSight -- Wave Table");
    SetTargetFPS(60);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "wave table", expectedOutput: "WaveTable: 5" },
      { id: "g2", description: "pattern", expectedOutput: "Pattern: wave-table" },
      { id: "g3", description: "gate A", expectedOutput: "GATE A: PASSED" },
    ],
    hints: [
      "getWaveConfig: int idx=(wave-1)%WAVE_TABLE_SIZE; return wave_table[idx];",
      "The starter's stub returns wave_table[0] — just fix the index to use modular arithmetic.",
    ],
    estimatedMinutes: 20,
  },
};
