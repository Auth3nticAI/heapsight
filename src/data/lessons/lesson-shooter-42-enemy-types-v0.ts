...
export const lessonShooter42: Lesson = {
  id: "shooter-42-enemy-types-v0",
  ...
  part1: {
    title: "Concept: Enemy Types v0",
    type: "concept",
    instructions: `# Enemy Types v0

## 3 Enemy Types
- **BASIC** (0): standard speed and size
- **FAST** (1): 1.8x speed, smaller
- **TANK** (2): 0.5x speed, larger

## Your Task
\`\`\`
EnemyTypes: 3
Type 0: BASIC speed 1.0x
...
float getSpeedMult(int type){
    // TODO: 0->1.0, 1->1.8, 2->0.5
    return 1.0f;
}
int main(){
    cout<<"EnemyTypes: 3"<<endl;
    float m[3]={getSpeedMult(0),getSpeedMult(1),getSpeedMult(2)};
    const char* names[]={"+BASIC","FAST","TANK"};
    for(int i=0;i<3;i++)
        cout<<"Type "<<i<<": "<<(i==0?"BASIC":i==1?"FAST":"TANK")<<" speed "<<m[i]<<"x"<<endl;
    cout<<"Pattern: enemy-types"<<endl;
    return 0;
}`,
    solutionCode: `#include <iostream>
using namespace std;
float getSpeedMult(int type){
    if(type==1) return 1.8f; if(type==2) return 0.5f; return 1.0f;
}
int main(){
    cout<<"EnemyTypes: 3"<<endl;
    for(int i=0;i<3;i++)
        cout<<"Type "<<i<<": "<<(i==0?"BASIC":i==1?"FAST":"TANK")<<" speed "<<getSpeedMult(i)<<"x"<<endl;
    cout<<"Pattern: enemy-types"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "3 types", expectedOutput: "EnemyTypes: 3" },
      { id: "t2", description: "basic speed", expectedOutput: "Type 0: BASIC speed 1x" },
  ...
  part2: {
    title: "Build: Enemy Types v0",
    type: "game_builder",
    instructions: `# Build: Enemy Types v0

## Your Task
**TODO 1** — Implement getEnemySpeed(int type, float base): return base*1.8 for FAST, base*0.5 for TANK, base for BASIC.

## Did It Work?
Enemies visually differ: FAST enemies zip down screen, TANK enemies lumber slowly. Orange=FAST, Red=TANK, Blue=BASIC.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
// Enemy types: 0=BASIC, 1=FAST, 2=TANK
// TODO 1: implement getEnemySpeed(int type, float base)
// FAST (1): base*1.8, TANK (2): base*0.5, BASIC (0): base
float getEnemySpeed(int type, float base) {
    return base; // fix this
}
int getEnemySize(int type){ return 20+type*4; }
Color getEnemyColor(int type){
    if(type==1) return ORANGE; if(type==2) return RED; return BLUE;
}
void hudSystem(World& w){
    DrawText("** ENEMY TYPES: 3 **",SCREEN_W/2-180,8,18,ORANGE);
...
int main(){
    InitWindow(SCREEN_W,SCREEN_H,"HeapSight -- Enemy Types");
    SetTargetFPS(60);
...
// Enemy types: 0=BASIC, 1=FAST, 2=TANK
float getEnemySpeed(int type, float base) {
    if(type==1) return base*1.8f;
    if(type==2) return base*0.5f;
    return base;
}
int getEnemySize(int type) {
    if(type==2) return 32;
    return 20+type*4;
}
Color getEnemyColor(int type) {
...
int main(){
    ...
    tests: [
      { id: "g1", description: "enemy types", expectedOutput: "EnemyTypes: 3" },
      { id: "g2", description: "pattern", expectedOutput: "Pattern: enemy-types" },
