...
export const lessonShooter46: Lesson = {
  id: "shooter-46-score-multiplier",
  ...
  part1: {
    title: "Concept: Score Multiplier",
    type: "concept",
    instructions: `# Score Multiplier

## Combo-Based Scoring
Each kill increments combo_hits. Multiplier = 1 + combo_hits / 10 (integer division). Wave clear resets combo to 0.
\`\`\`cpp
...
int getMultiplier(int combo){
    // TODO: return 1 + combo/10
    return 1;
}
int main(){
    cout<<"Multiplier: 1x"<<endl;
    int combos[]={0,10,20};
    for(int i=0;i<3;i++)
        cout<<"Combo "<<combos[i]<<": "<<getMultiplier(combos[i])<<"x"<<endl;
    cout<<"Pattern: score-multiplier"<<endl;
    return 0;
}`,
    solutionCode: `#include <iostream>
using namespace std;
int getMultiplier(int combo){ return 1+combo/10; }
int main(){
    cout<<"Multiplier: 1x"<<endl;
    int combos[]={0,10,20};
    for(int i=0;i<3;i++)
    ...
    tests: [
      { id: "t1", description: "multiplier line", expectedOutput: "Multiplier: 1x" },
      { id: "t2", description: "combo 10", expectedOutput: "Combo 10: 2x" },
  ...
  part2: {
    title: "Build: Score Multiplier",
    type: "game_builder",
    instructions: `# Build: Score Multiplier

## What's Already Here
collisionSystem already computes: int mult=1+w.combo_hits/10; w.score+=100*mult; w.combo_hits++;
waveSystem resets combo_hits=0 on wave clear.

## Did It Work?
HUD shows current multiplier. Kill 10 in a row without clearing the wave to see 2x appear.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** SCORE MULTIPLIER **",SCREEN_W/2-180,8,18,GOLD);
    ...
    int mult=1+w.combo_hits/10;
    DrawText(TextFormat("Mult: %dx",mult),10,198,16,GOLD);
}
...
int main(){
    InitWindow(SCREEN_W,SCREEN_H,"HeapSight -- Score Multiplier");
    SetTargetFPS(60);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "multiplier", expectedOutput: "Multiplier: 1x" },
      { id: "g2", description: "pattern", expectedOutput: "Pattern: score-multiplier" },
