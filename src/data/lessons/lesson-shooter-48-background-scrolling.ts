...
export const lessonShooter48: Lesson = {
  id: "shooter-48-background-scrolling",
  ...
  part1: {
    title: "Concept: Background Scrolling",
    type: "concept",
    instructions: `# Background Scrolling

## Dynamic Starfield
Instead of 12 static dots, 20 stars scroll downward at different speeds creating a parallax effect.
\`\`\`cpp
void updateScrollStars() {
    for(int i=0;i<NUM_SCROLL_STARS;i++){
        scroll_star_y[i] += scroll_star_spd[i] * FIXED_DT;
        if(scroll_star_y[i] > SCREEN_H) {
            scroll_star_y[i] = 0;
            scroll_star_x[i] = (float)rng_range(SCREEN_W);
        }
    }
}
\`\`\`
...
const int N=20;
float sy[N]={};
float sspd[N]={30,50,40,60,20,45,35,55,25,65,30,50,40,60,20,45,35,55,25,65};
const float DT=1.0f/60;
void updateStars(){
    for(int i=0;i<N;i++){
        sy[i]+=sspd[i]*DT;
        // TODO: if sy[i]>450, reset to 0
    }
}
int main(){
    cout<<"Stars: "<<N<<" scrolling"<<endl;
    for(int t=0;t<60;t++) updateStars();
    cout<<"Pattern: bg-scroll"<<endl;
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "star count", expectedOutput: "Stars: 20 scrolling" },
  ...
  part2: {
    title: "Build: Background Scrolling",
    type: "game_builder",
    instructions: `# Build: Background Scrolling

## Your Task
**TODO 1** — Implement updateScrollStars(): move each star down by spd*FIXED_DT, wrap to y=0 when off screen.

**TODO 2** — Call updateScrollStars() in gameFrame (in the physics loop).

**TODO 3** — In renderSystem, replace the static DrawRectangle stars with drawScrollStars();

## Did It Work?
Stars flow downward at varying speeds. Background feels alive.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
const int NUM_SCROLL_STARS = 20;
float scroll_star_x[NUM_SCROLL_STARS];
float scroll_star_y[NUM_SCROLL_STARS];
float scroll_star_spd[NUM_SCROLL_STARS];

void initScrollStars(){
    rng_state=99;
    for(int i=0;i<NUM_SCROLL_STARS;i++){
        scroll_star_x[i]=(float)rng_range(SCREEN_W);
        scroll_star_y[i]=(float)rng_range(SCREEN_H);
        scroll_star_spd[i]=20.0f+rng_range(60);
    }
}
// TODO 1: implement updateScrollStars()
// For each star: y += spd*FIXED_DT; if y>SCREEN_H, reset y=0 and randomize x
void updateScrollStars(){ }

void drawScrollStars(){
    for(int i=0;i<NUM_SCROLL_STARS;i++)
        DrawRectangle((int)scroll_star_x[i],(int)scroll_star_y[i],2,2,WHITE);
}
void hudSystem(World& w){
    DrawText("** BG SCROLL: 20 STARS **",SCREEN_W/2-180,8,18,SKYBLUE);
...
void renderSystem(World& w){
    ...
    // TODO 3: replace static stars above with drawScrollStars();
    for(int i=0;i<MAX_ENEMIES;i++) if(w.enemy_active[i]){
...
void gameFrame(World& w,float& acc){
    while(acc>=FIXED_DT){
        ...
        collisionSystem(w);
        waveSystem(w);
        // TODO 2: call updateScrollStars() here
        acc-=FIXED_DT;
    }
}
int main(){
    InitWindow(SCREEN_W,SCREEN_H,"HeapSight -- BG Scroll");
    SetTargetFPS(60);
...
void updateScrollStars() {
    for(int i=0;i<NUM_SCROLL_STARS;i++){
        scroll_star_y[i]+=scroll_star_spd[i]*FIXED_DT;
        if(scroll_star_y[i]>SCREEN_H){scroll_star_y[i]=0;scroll_star_x[i]=(float)rng_range(SCREEN_W);}
    }
}
void drawScrollStars(){
    for(int i=0;i<NUM_SCROLL_STARS;i++)
...
int main(){
    ...
    tests: [
      { id: "g1", description: "stars", expectedOutput: "Stars: 20 scrolling" },
      { id: "g2", description: "pattern", expectedOutput: "Pattern: bg-scroll" },
      { id: "g3", description: "gate A", expectedOutput: "GATE A: PASSED" },
    ],
    hints: [
      "updateScrollStars: y+=spd*FIXED_DT; if(y>SCREEN_H){y=0; x=rng_range(SCREEN_W);}",
      "Call updateScrollStars() in gameFrame after waveSystem(w).",
      "In renderSystem: replace the 12 static DrawRectangle lines with drawScrollStars();",
    ],
    estimatedMinutes: 20,
  },
};
