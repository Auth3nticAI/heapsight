...
export const lessonShooter49: Lesson = {
  id: "shooter-49-lives-system",
  title: "Lives System",
  ...
  part1: {
    title: "Concept: Lives System",
    type: "concept",
    instructions: `# Lives System

## 3 Lives, Game Over, Restart
- Enemy-ship collision: player_hp--. When HP hits 0: lives--, reset HP.
- When lives hits 0: game_over=true.
- Press R to restart.
...
void takeDamage(){
    hp--;
    if(hp<=0){lives--;hp=3;if(lives<=0)game_over=true;}
}
int main(){
    cout<<"Lives: "<<lives<<endl;
    takeDamage();takeDamage();takeDamage();
    // TODO: print Game over: yes if game_over is true
    cout<<"Pattern: lives-system"<<endl;
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "lives", expectedOutput: "Lives: 3" },
  ...
  part2: {
    title: "Build: Lives System",
    type: "game_builder",
    instructions: `# Build: Lives System

## What's Already Here
collisionSystem handles enemy-ship collision: it reduces player_hp and then decrements lives when HP hits 0.

## Your Task
**TODO 1** — In gameFrame, handle game_over==true: check for KEY_R to restart (reset all state), then return early.

**TODO 2** — After waveSystem(w), check if(w.lives<=0) set w.game_over=true.

## Did It Work?
Die 3 times (get hit by enemies), see GAME OVER. Press R to restart.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** LIVES SYSTEM: 3 LIVES **",SCREEN_W/2-180,8,18,PINK);
    ...
    if(w.game_over) DrawText("GAME OVER -- press R",SCREEN_W/2-130,SCREEN_H/2,28,RED);
}
...
void gameFrame(World& w,float& acc){
    // TODO 1: check w.game_over; if true, handle R-key restart and early return
    ...
    while(acc>=FIXED_DT){
        ...
        collisionSystem(w);
        waveSystem(w);
        // TODO 2: if w.lives<=0, set w.game_over=true
        acc-=FIXED_DT;
    }
}
int main(){
    InitWindow(SCREEN_W,SCREEN_H,"HeapSight -- Lives System");
    SetTargetFPS(60);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "lives", expectedOutput: "Lives: 3" },
      { id: "g2", description: "pattern", expectedOutput: "Pattern: lives-system" },
      { id: "g3", description: "gate A", expectedOutput: "GATE A: PASSED" },
    ],
    hints: [
      "TODO 1: if(w.game_over){ if(IsKeyPressed(KEY_R)){ reset w.score,wave,lives,hp,game_over; clear arrays; spawnWave; } return; }",
      "TODO 2: if(w.lives<=0) w.game_over=true; — add this after waveSystem(w).",
    ],
    estimatedMinutes: 20,
  },
};
