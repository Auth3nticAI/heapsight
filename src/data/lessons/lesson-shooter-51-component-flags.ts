...
export const lessonShooter51: Lesson = {
  id: "shooter-51-component-flags",
  title: "Component Flags",
  description: "Add bitmask component flags for entity capability queries.",
  order: 51,
  xpReward: 100,
  tier: "pro",
  concepts: ["component flags", "bitmask", "entity capabilities", "ECS queries"],
  part1: {
    title: "Concept: Component Flags",
    type: "concept",
    instructions: `# Component Flags

## Mental Model
Entities are just IDs. What makes an entity a "ship" vs an "enemy" is which component flags it has. A bitmask encodes capabilities: bit 0 = position, bit 1 = velocity, bit 2 = renderable. Systems check flags before processing.

## What Breaks Without This
Without component flags, every system must check every entity. With flags, a movement system skips entities without CF_VELOCITY. 1000 static decorations never enter the movement loop.

## The Fix: Component Flags
Implement the component flags concept in console output, then apply it to the game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Using separate booleans for each component presence.** Booleans waste memory and require individual checks. A single bitmask (uint32_t flags) stores 32 component flags in one integer. Check with bitwise AND: if (flags & HAS_VELOCITY).

## Elite Insight
EnTT and flecs use bitmask-based archetype queries — each component type has a bit, and queries check the bitmask in one operation. Your component flags implement the same O(1) "does this entity have component X?" check.

## Systems Thinking Connection
The RPG and Crawler use simpler entity type enums. Your bitmask approach is the most ECS-native across all paths — it enables arbitrary component combinations without predefined entity types.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    unsigned int flags = 0;
    // TODO: set bits 0,1,2 using |= (1<<N)
    int bits = 0;
    for(int i=0;i<8;i++) if(flags & (1<<i)) bits++;
    cout<<"CompFlags: "<<bits<<" bits"<<endl;
    // TODO: print Flagged: 5 entities
    return 0;
}`,
...
int main(){
    ...
    flags |= (1<<0); // position
    flags |= (1<<1); // velocity
    flags |= (1<<2); // renderable
    int bits = 0;
    for(int i=0;i<8;i++) if(flags & (1<<i)) bits++;
    cout<<"CompFlags: "<<bits<<" bits"<<endl;
    int flagged = 5;
    cout<<"Flagged: "<<flagged<<" entities"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "component-flags line 1", expectedOutput: "CompFlags: 7 bits" },
      { id: "t2", description: "component-flags line 2", expectedOutput: "Flagged: 5 entities" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Component Flags",
    type: "game_builder",
    instructions: `# Build: Component Flags

Add component flags to the running game.

## Did It Work?
Console shows the new component flags output. Canvas shows the updated game with the new feature active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L51: Advanced ECS **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "component-flags line 1", expectedOutput: "CompFlags: 7 bits" },
      { id: "g2", description: "component-flags line 2", expectedOutput: "Flagged: 5 entities" },
    ],
    hints: [
