...
export const lessonShooter52: Lesson = {
  id: "shooter-52-system-ordering-rules",
  title: "System Ordering Rules",
  description: "Enforce explicit system execution order with dependency tracking.",
  order: 52,
  xpReward: 100,
  tier: "pro",
  concepts: ["system ordering", "dependencies", "update pipeline", "execution order"],
  part1: {
    title: "Concept: System Ordering Rules",
    type: "concept",
    instructions: `# System Ordering Rules

## Mental Model
Systems must run in a specific order. Input before movement, movement before collision, collision before rendering. Get this wrong and bullets detect hits one frame late, or enemies move after death.

## What Breaks Without This
If collision runs before movement, bullets check positions from last frame. If rendering runs before collision, dead enemies flash for one frame before disappearing.

## The Fix: System Ordering Rules
Implement the system ordering rules concept in console output, then apply it to the game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Assuming system order does not matter.** If the collision system reads positions that the movement system has not yet updated, it detects collisions at stale positions. Document and enforce the dependency graph: movement before collision, collision before cleanup.

## Elite Insight
Unity defines script execution order numerically (-100 before 0 before 100). Unreal uses tick groups with explicit dependencies. Your ordered system array is the manual equivalent of the same scheduling guarantee.

## Systems Thinking Connection
RPG enforces update order (L15). Platformer enforces pipeline order (L35). The ordering problem is universal: when systems share data, the order they run determines correctness. Every path enforces this explicitly.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    const char* order[] = {"input","move","collide","wave","render"};
    // TODO: print SysOrder: input->move->collide->wave->render
    // TODO: print Pipeline: 5 systems ordered
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "system-ordering-rules line 1", expectedOutput: "SysOrder: input->move->collide->wave->render" },
      { id: "t2", description: "system-ordering-rules line 2", expectedOutput: "Pipeline: 5 systems ordered" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: System Ordering Rules",
    type: "game_builder",
    instructions: `# Build: System Ordering Rules

Add system ordering rules to the running game.

## Did It Work?
Console shows the new system ordering rules output. Canvas shows the updated game with the new feature active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L52: Advanced ECS **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "system-ordering-rules line 1", expectedOutput: "SysOrder: input->move->collide->wave->render" },
      { id: "g2", description: "system-ordering-rules line 2", expectedOutput: "Pipeline: 5 systems ordered" },
    ],
    hints: [
