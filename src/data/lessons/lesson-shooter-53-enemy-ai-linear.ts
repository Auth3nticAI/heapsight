...
export const lessonShooter53: Lesson = {
  id: "shooter-53-enemy-ai-linear",
  title: "Enemy AI: Linear",
  description: "Enemies aim directly at the player position.",
  order: 53,
  xpReward: 100,
  tier: "pro",
  concepts: ["enemy AI", "targeting", "linear pursuit", "behavior system"],
  part1: {
    title: "Concept: Enemy AI: Linear",
    type: "concept",
    instructions: `# Enemy AI: Linear

## Mental Model
A dumb enemy falls straight down. A smart enemy aims at you. Linear pursuit: compute the direction vector from enemy to player, normalize it, scale by speed. The enemy homes in. Simple math, dramatic gameplay.

## What Breaks Without This
Enemies that only fall straight are boring. The player develops a single strategy (sit at bottom, shoot up) and never adapts.

## The Fix: Enemy AI: Linear
Implement the enemy ai: linear concept in console output, then apply it to the game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Normalizing the direction vector without checking for zero length.** If the enemy is exactly at the player position, the direction is (0,0) and normalizing divides by zero. Always check length > epsilon before normalizing.

## Elite Insight
Linear pursuit — moving directly toward the target at constant speed — is the simplest AI behavior. It is also the most predictable and the easiest to dodge. Professional games use it for weak enemies and replace it with smarter behaviors for elites.

## Systems Thinking Connection
RPG enemies chase on a grid (L06). Crawler enemies chase in 3D (L37). Your linear pursuit is the 2D continuous-space version. All paths start with "move toward player" and later add complexity (patrol, flank, flee).`,
    starterCode: `#include <iostream>
using namespace std;
#include <cmath>
int main(){
    float ex=200,ey=50,px=320,py=430;
    // TODO: compute dx=px-ex, dy=py-ey
    // TODO: print AI: linear aim dx=120 dy=380
...
int main(){
    ...
    cout<<"AI: linear aim dx="<<(int)dx<<" dy="<<(int)dy<<endl;
    cout<<"LinearAI: 5 enemies tracking"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "enemy-ai-linear line 1", expectedOutput: "AI: linear aim dx=-120 dy=380" },
      { id: "t2", description: "enemy-ai-linear line 2", expectedOutput: "LinearAI: 5 enemies tracking" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Enemy AI: Linear",
    type: "game_builder",
    instructions: `# Build: Enemy AI: Linear

Add enemy ai: linear to the running game.

## Did It Work?
Console shows the new enemy ai: linear output. Canvas shows the updated game with the new feature active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L53: Advanced ECS **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "enemy-ai-linear line 1", expectedOutput: "AI: linear aim dx=-120 dy=380" },
      { id: "g2", description: "enemy-ai-linear line 2", expectedOutput: "LinearAI: 5 enemies tracking" },
    ],
    hints: [
