...
export const lessonShooter59: Lesson = {
  id: "shooter-59-entity-tags-v0",
  title: "Entity Tags v0",
  description: "Tag system for entity classification and filtered queries.",
  order: 59,
  xpReward: 100,
  tier: "pro",
  concepts: ["entity tags", "classification", "filtered queries", "tag bitmask"],
  part1: {
    title: "Concept: Entity Tags v0",
    type: "concept",
    instructions: `# Entity Tags v0

## Mental Model
Entity tags classify entities for filtered queries. Player=1, Enemy=2, Bullet=4, Powerup=8. Using powers of two lets you combine tags with bitwise OR and test with AND. A system that processes "all enemies" checks (tag & TAG_ENEMY).

## What Breaks Without This
Without tags, you need separate code paths for "find all enemies" vs "find all bullets." Tags unify entity queries.

## The Fix: Entity Tags v0
Implement the entity tags v0 concept in console output, then apply it to the game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Using string tags instead of enum/bitmask tags.** String comparison is slow and error-prone ("Enemy" vs "enemy" vs "ENEMY"). Use integer enum tags or bitmask flags for O(1) tag checks.

## Elite Insight
ECS engines use archetype tags for filtered queries: "give me all entities with Position + Velocity but not Dead." Your tag bitmask enables the same pattern — filter entities by capability, not by type name.

## Systems Thinking Connection
RPG uses entity type enums. Crawler uses type IDs for enemies. Bitmask-based entity classification scales to complex queries (all enemies with shields that are not dead) without string comparisons or inheritance.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    // TODO: define TAG_PLAYER=1, TAG_ENEMY=2, TAG_BULLET=4, TAG_POWERUP=8
    // TODO: print Tags: player=1 enemy=2 bullet=4 powerup=8
    // TODO: print Tagged: 14 entities
    return 0;
}`,
    solutionCode: `#include <iostream>
using namespace std;
int main(){
    unsigned int TAG_PLAYER=1, TAG_ENEMY=2, TAG_BULLET=4, TAG_POWERUP=8;
    cout<<"Tags: player="<<TAG_PLAYER<<" enemy="<<TAG_ENEMY<<" bullet="<<TAG_BULLET<<" powerup="<<TAG_POWERUP<<endl;
    int tagged=1+5+4+4;
    cout<<"Tagged: "<<tagged<<" entities"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "entity-tags-v0 line 1", expectedOutput: "Tags: player=1 enemy=2 bullet=4 powerup=8" },
      { id: "t2", description: "entity-tags-v0 line 2", expectedOutput: "Tagged: 14 entities" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Entity Tags v0",
    type: "game_builder",
    instructions: `# Build: Entity Tags v0

Add entity tags v0 to the running game.

## Did It Work?
Console shows the new entity tags v0 output. Canvas shows the updated game with the new feature active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L59: Advanced ECS **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "entity-tags-v0 line 1", expectedOutput: "Tags: player=1 enemy=2 bullet=4 powerup=8" },
      { id: "g2", description: "entity-tags-v0 line 2", expectedOutput: "Tagged: 14 entities" },
    ],
    hints: [
