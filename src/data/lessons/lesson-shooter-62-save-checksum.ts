...
export const lessonShooter62: Lesson = {
  id: "shooter-62-save-checksum",
  title: "Save Checksum",
  description: "Add integrity checksum to detect save corruption.",
  order: 62,
  xpReward: 200,
  tier: "pro",
  concepts: ["checksum", "data integrity", "CRC", "corruption detection"],
  part1: {
    title: "Concept: Save Checksum",
    type: "concept",
    instructions: `# Save Checksum

## Mental Model
Data corruption happens. Bit flips, truncated writes, cosmic rays. A checksum catches it. XOR the key fields, store the result. On load, recompute and compare.

## What Breaks Without This
Without checksums, corrupted saves load silently and cause mysterious crashes or wrong state.

## The Fix: Save Checksum
Implement the save checksum concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Computing the checksum over the entire file including the checksum field itself.** The checksum must be computed over the data, then appended. On load, compute the checksum over the data portion and compare to the stored value.

## Elite Insight
CRC32, used in ZIP files and network protocols, detects accidental corruption with 99.998% reliability. Your save checksum follows the same pattern: hash the data, store the hash, verify on load.

## Systems Thinking Connection
RPG and Crawler implement save checksums too. The Platformer stores checkpoint integrity hashes. Data integrity verification is a universal requirement for any system that persists state to disk.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    unsigned int seed=42, score=0, wave=1;
    unsigned int checksum = seed ^ score ^ wave;
    // TODO: print Checksum: seed=42 score=0 wave=1 -> {checksum}
    // TODO: print Verify: checksum match=true
    return 0;
}`,
...
int main(){
    unsigned int seed=42, score=0, wave=1;
    unsigned int checksum = seed ^ score ^ wave;
    cout<<"Checksum: seed=42 score=0 wave=1 -> "<<checksum<<endl;
    cout<<"Verify: checksum match=true"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "save-checksum line 1", expectedOutput: "Checksum: seed=42 score=0 wave=1 -> 83" },
      { id: "t2", description: "save-checksum line 2", expectedOutput: "Verify: checksum match=true" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Save Checksum",
    type: "game_builder",
    instructions: `# Build: Save Checksum

Add save checksum to the running game.

## Did It Work?
Console shows the new save checksum output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L62: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "save-checksum line 1", expectedOutput: "Checksum: seed=42 score=0 wave=1 -> 83" },
      { id: "g2", description: "save-checksum line 2", expectedOutput: "Verify: checksum match=true" },
    ],
    hints: [
