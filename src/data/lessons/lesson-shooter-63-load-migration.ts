...
export const lessonShooter63: Lesson = {
  id: "shooter-63-load-migration",
  title: "Load Migration",
  description: "Handle older save versions with forward-compatible migration.",
  order: 63,
  xpReward: 200,
  tier: "pro",
  concepts: ["migration", "forward compatibility", "version handling", "schema upgrade"],
  part1: {
    title: "Concept: Load Migration",
    type: "concept",
    instructions: `# Load Migration

## Mental Model
Version 0 saves exist in the wild. Version 1 adds a lives field. Migration reads the version, fills in defaults for missing fields. Forward compatibility without data loss.

## What Breaks Without This
Without migration, old saves crash on load. Players must start over after every update.

## The Fix: Load Migration
Implement the load migration concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Failing to load old save files after a format change.** Players lose progress. Migration means: read old format, convert to new format, save in new format. Each version bump needs a migration function.

## Elite Insight
Database migrations (Rails, Django, Flyway) apply sequential transforms to upgrade schema. Game save migration is the same problem: each version change needs a transform function. Your version-checked loader follows the same pattern.

## Systems Thinking Connection
RPG (L24) and Crawler (L62) handle save migration. Every path faces the same challenge: game updates must not break existing save files. Version-checked deserialization with migration is the standard solution.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    int old_version = 0;
    int new_version = 1;
    int lives = 3; // added in v1
    // TODO: print Migrate: v0->v1 added lives=3
    // TODO: print LoadOK: version=1 migrated=true
    return 0;
}`,
...
int main(){
    ...
    int new_version = 1;
    int lives = 3;
    cout<<"Migrate: v0->v1 added lives="<<lives<<endl;
    cout<<"LoadOK: version="<<new_version<<" migrated=true"<<endl;
    return 0;
}`,
    tests: [
      { id: "t1", description: "load-migration line 1", expectedOutput: "Migrate: v0->v1 added lives=3" },
      { id: "t2", description: "load-migration line 2", expectedOutput: "LoadOK: version=1 migrated=true" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Load Migration",
    type: "game_builder",
    instructions: `# Build: Load Migration

Add load migration to the running game.

## Did It Work?
Console shows the new load migration output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L63: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "load-migration line 1", expectedOutput: "Migrate: v0->v1 added lives=3" },
      { id: "g2", description: "load-migration line 2", expectedOutput: "LoadOK: version=1 migrated=true" },
    ],
    hints: [
