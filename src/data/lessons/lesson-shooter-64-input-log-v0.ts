...
export const lessonShooter64: Lesson = {
  id: "shooter-64-input-log-v0",
  title: "Input Log v0",
  description: "Record per-frame input events for deterministic replay.",
  order: 64,
  xpReward: 200,
  tier: "pro",
  concepts: ["input log", "frame recording", "replay foundation", "input serialization"],
  part1: {
    title: "Concept: Input Log v0",
    type: "concept",
    instructions: `# Input Log v0

## Mental Model
Replay starts with recording. Every frame, log what keys were pressed. Store frame number + key bitmask. 256 frames of input fits in a tiny buffer.

## What Breaks Without This
Without input logging, replay is impossible. You cannot reproduce bugs or verify determinism.

## The Fix: Input Log v0
Implement the input log v0 concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Recording processed game actions instead of raw key states.** Replays must reproduce the exact input. If you record "fired bullet at (100, 50)" instead of "SPACE key down at frame 847," the replay cannot handle code changes that alter bullet behavior.

## Elite Insight
Doom recorded raw input as "tics" — one complete input snapshot per game frame. This made .lmp demo files tiny and perfectly reproducible. Your per-frame input log follows the same design from 1993.

## Systems Thinking Connection
RPG, Crawler, and Platformer all record raw input per frame. The input types differ (keyboard, mouse+keyboard, 2D, 3D) but the principle is identical: capture raw input, not derived actions.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    unsigned int input_log[3] = {0x0000, 0x0001, 0x0004};
    cout<<"InputLog: frame=0 keys=0x0000"<<endl;
    // TODO: print Logged: 3 frames recorded
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "input-log-v0 line 1", expectedOutput: "InputLog: frame=0 keys=0x0000" },
      { id: "t2", description: "input-log-v0 line 2", expectedOutput: "Logged: 3 frames recorded" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Input Log v0",
    type: "game_builder",
    instructions: `# Build: Input Log v0

Add input log v0 to the running game.

## Did It Work?
Console shows the new input log v0 output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L64: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "input-log-v0 line 1", expectedOutput: "InputLog: frame=0 keys=0x0000" },
      { id: "g2", description: "input-log-v0 line 2", expectedOutput: "Logged: 3 frames recorded" },
    ],
    hints: [
