...
export const lessonShooter66: Lesson = {
  id: "shooter-66-replay-playback-pipeline",
  title: "Replay Playback Pipeline",
  description: "Input source swaps between live and recorded replay.",
  order: 66,
  xpReward: 200,
  tier: "pro",
  concepts: ["playback pipeline", "input source swap", "replay mode", "source abstraction"],
  part1: {
...
## Mental Model
The input system reads from keyboard OR from the replay log. Swap the source, keep the rest of the pipeline identical. The game does not know the difference.

## What Breaks Without This
Without input source abstraction, replay requires duplicating the entire game loop.

## The Fix: Replay Playback Pipeline
Implement the replay playback pipeline concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Mixing live input with replay input.** During playback, the game should read from the input log, not from the keyboard. Swap the input source: live mode reads hardware, replay mode reads the log. The rest of the game does not know the difference.

## Elite Insight
The "input source abstraction" pattern lets the same game loop accept live input, recorded input, or network input. Professional engines use this for replays, tutorials, and AI bots. Your source swap follows the same pattern.

## Systems Thinking Connection
RPG, Crawler, and Platformer implement the same input source swap. The abstraction is identical: the game loop reads from an InputSnapshot interface, and the source (live, replay, network) is swapped externally.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    bool replay_mode = false;
    // TODO: swap to replay mode, print InputSource: live->replay swap
    // TODO: print Playback: frame 0 injected
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "replay-playback-pipeline line 1", expectedOutput: "InputSource: live->replay swap" },
      { id: "t2", description: "replay-playback-pipeline line 2", expectedOutput: "Playback: frame 0 injected" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Replay Playback Pipeline",
    type: "game_builder",
    instructions: `# Build: Replay Playback Pipeline

Add replay playback pipeline to the running game.

## Did It Work?
Console shows the new replay playback pipeline output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L66: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "replay-playback-pipeline line 1", expectedOutput: "InputSource: live->replay swap" },
      { id: "g2", description: "replay-playback-pipeline line 2", expectedOutput: "Playback: frame 0 injected" },
    ],
    hints: [
