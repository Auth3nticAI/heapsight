...
export const lessonShooter67: Lesson = {
  id: "shooter-67-determinism-pitfalls",
  title: "Determinism Pitfalls",
  description: "Stable iteration order prevents divergence in replay.",
  order: 67,
  xpReward: 200,
  tier: "pro",
  concepts: ["determinism", "iteration order", "stable sort", "divergence"],
  part1: {
...
## Mental Model
Floating point arithmetic is not associative. a+b+c != c+b+a at the bit level. Iteration order must be stable — always process entity 0 before entity 1. No sorting by floating point values.

## What Breaks Without This
Unstable iteration causes replay divergence. Entity 3 processed before entity 1 on one run, reversed on replay.

## The Fix: Determinism Pitfalls
Implement the determinism pitfalls concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Using std::sort without a stable comparator.** std::sort is not guaranteed to be stable — equal elements can swap order between runs. Use std::stable_sort or add a tiebreaker (entity ID) to the comparator.

## Elite Insight
Floating-point determinism is the #1 cause of lockstep desync in networked games. Different compilers optimize x87 vs SSE differently. Your pitfall catalog documents the traps that catch every game developer.

## Systems Thinking Connection
RPG, Crawler, and Platformer face the same pitfalls. Iteration order, unstable sort, and floating-point precision affect every path equally. The determinism discipline is shared across all game genres.`,
...
int main(){
    ...
    // TODO: print IterOrder: stable across 5 entities
    // TODO: print Determinism: no float drift
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "determinism-pitfalls line 1", expectedOutput: "IterOrder: stable across 5 entities" },
      { id: "t2", description: "determinism-pitfalls line 2", expectedOutput: "Determinism: no float drift" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Determinism Pitfalls",
    type: "game_builder",
    instructions: `# Build: Determinism Pitfalls

Add determinism pitfalls to the running game.

## Did It Work?
Console shows the new determinism pitfalls output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L67: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "determinism-pitfalls line 1", expectedOutput: "IterOrder: stable across 5 entities" },
      { id: "g2", description: "determinism-pitfalls line 2", expectedOutput: "Determinism: no float drift" },
    ],
    hints: [
