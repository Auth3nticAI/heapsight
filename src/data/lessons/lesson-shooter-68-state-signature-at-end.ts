...
export const lessonShooter68: Lesson = {
  id: "shooter-68-state-signature-at-end",
  title: "State Signature at End",
  description: "Final hash compared between live and replay runs.",
  order: 68,
  xpReward: 200,
  tier: "pro",
  concepts: ["state signature", "end hash", "verification", "hash comparison"],
  part1: {
    title: "Concept: State Signature at End",
    type: "concept",
    instructions: `# State Signature at End

## Mental Model
At the end of a run, hash the entire world state: score, wave, lives, entity positions. Compare live hash to replay hash. If they match, determinism is proven.

## What Breaks Without This
Without end-state signatures, you cannot detect slow drift. Two runs may diverge by frame 1000 but look identical at frame 10.

## The Fix: State Signature at End
Implement the state signature at end concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Only hashing entity positions and forgetting score, lives, and wave number.** Every gameplay-relevant value must be in the signature. If the score diverges but the hash does not catch it, your signature is incomplete.

## Elite Insight
Age of Empires computes a per-frame hash of all game state for multiplayer sync checking. Any divergence triggers a resync. Your end-of-frame signature serves the same purpose for single-player replay verification.

## Systems Thinking Connection
RPG (L23), Crawler (L64), and Platformer all compute state signatures. The hash function and included fields differ, but the purpose is identical: detect state divergence between recording and playback.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    unsigned int hash = 0x002A;
    int score=0, wave=1;
    // TODO: print EndSig: hash=0x002A score=0 wave=1
    // TODO: print SigMatch: live==replay true
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "state-signature-at-end line 1", expectedOutput: "EndSig: hash=0x002A score=0 wave=1" },
      { id: "t2", description: "state-signature-at-end line 2", expectedOutput: "SigMatch: live==replay true" },
    ],
    hints: [
  ...
  part2: {
    title: "Build: State Signature at End",
    type: "game_builder",
    instructions: `# Build: State Signature at End

Add state signature at end to the running game.

## Did It Work?
Console shows the new state signature at end output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L68: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "state-signature-at-end line 1", expectedOutput: "EndSig: hash=0x002A score=0 wave=1" },
      { id: "g2", description: "state-signature-at-end line 2", expectedOutput: "SigMatch: live==replay true" },
    ],
    hints: [
