...
export const lessonShooter69: Lesson = {
  id: "shooter-69-replay-verification-test",
  title: "Replay Verification Test",
  description: "Automated mismatch detection triggers error on divergence.",
  order: 69,
  xpReward: 200,
  tier: "pro",
  concepts: ["replay test", "mismatch detection", "automated verification", "regression"],
  part1: {
...
## Mental Model
Run N frames live. Record inputs. Replay those N frames. Compare state signatures at every frame. Any mismatch triggers an error with the diverging frame number.

## What Breaks Without This
Without automated verification, you manually compare replay runs. That does not scale.

## The Fix: Replay Verification Test
Implement the replay verification test concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Running the verification only once.** A test that passes with one seed may fail with another. Run verification with at least 5 different seeds and gameplay styles to catch determinism bugs that depend on specific sequences.

## Elite Insight
Professional QA runs hundreds of automated replay tests nightly — each with different seeds, durations, and player behaviors. Your verification test should cover diverse scenarios, not just the happy path.

## Systems Thinking Connection
RPG, Crawler, and Platformer run the same replay verification tests. The testing pattern is path-universal: record diverse sessions, replay them all, assert zero signature mismatches.`,
...
Add replay verification test to the running game.

## Did It Work?
Console shows the new replay verification test output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** L69: Replay & Save **",SCREEN_W/2-180,8,18,GOLD);
