...
export const lessonShooter73: Lesson = {
  id: "shooter-73-spatial-grid-tuning",
  title: "Spatial Grid Tuning",
  description: "Optimized cell size for collision.",
  order: 73,
  xpReward: 100,
  tier: "pro",
  concepts: ["spatial grid", "cell size", "collision tuning"],
  part1: {
    title: "Concept: Spatial Grid Tuning",
    type: "concept",
    instructions: `# Spatial Grid Tuning

## Mental Model
Performance is not about guessing. It is about measuring. Optimized cell size for collision.

## What Breaks Without This
Wrong cell size means either too many cells (overhead) or too few (N-squared checks inside cells).

## The Fix: Spatial Grid Tuning
Implement the spatial grid tuning concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Using a 1x1 pixel cell size.** Too-small cells waste memory and iteration time. Too-large cells put too many entities per cell. Optimal cell size is roughly 2x the average entity size — enough to hold neighbors in adjacent cells.

## Elite Insight
Spatial grid tuning is a classic space-time tradeoff. Smaller cells = more cells to check, fewer entities per cell. Larger cells = fewer cells to check, more entities per cell. Profile to find the sweet spot.

## Systems Thinking Connection
The Crawler tunes spatial hash bucket size (L54). RPG tunes grid cell size (L73). Every path with spatial acceleration faces the same tuning problem — cell size determines the performance curve.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"Spatial Grid Tuning"<<endl;
    // TODO: print all spatial grid tuning diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "spatial-grid-tuning line 1", expectedOutput: "Spatial Grid Tuning", isPattern: false },
      { id: "t2", description: "spatial-grid-tuning line 2", expectedOutput: "CollisionQueries: grid vs brute", isPattern: false },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Spatial Grid Tuning",
    type: "game_builder",
    instructions: `# Build: Spatial Grid Tuning

Add spatial grid tuning to the running game.

## Did It Work?
Console shows the new spatial grid tuning output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** HOT PATH AUDIT **",SCREEN_W/2-180,8,18,GOLD);
...
// TODO: Add spatial grid tuning — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** SPATIAL GRID TUNING **",SCREEN_W/2-180,8,18,GOLD);
    DrawText(TextFormat("Score: %d x%d",w.score,1+w.combo_hits/10),10,38,18,WHITE);
    DrawText(TextFormat("Wave:  %d",w.wave),10,62,18,WHITE);
    DrawText(TextFormat("HP: %d/%d",w.player_hp,w.player_max_hp),10,86,18,WHITE);
    DrawText(TextFormat("Lives: %d",w.lives),10,110,18,PINK);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "spatial-grid-tuning line 1", expectedOutput: "Spatial Grid Tuning", isPattern: false },
      { id: "g2", description: "spatial-grid-tuning output", expectedOutput: "GridCells: 10x6=60", isPattern: false },
      { id: "g3", description: "spatial-grid-tuning complete", expectedOutput: "CollisionQueries: grid vs brute", isPattern: false },
