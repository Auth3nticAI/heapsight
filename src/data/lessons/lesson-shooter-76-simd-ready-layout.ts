...
export const lessonShooter76: Lesson = {
  id: "shooter-76-simd-ready-layout",
  title: "SIMD-Ready Layout",
  description: "Aligned SoA arrays.",
  order: 76,
  xpReward: 100,
  tier: "pro",
  concepts: ["SIMD", "alignas", "data alignment", "cache lines"],
  part1: {
    title: "Concept: SIMD-Ready Layout",
    type: "concept",
    instructions: `# SIMD-Ready Layout

## Mental Model
Performance is not about guessing. It is about measuring. Aligned SoA arrays.

## What Breaks Without This
Misaligned data causes the CPU to load two cache lines instead of one. Twice the memory bandwidth.

## The Fix: SIMD-Ready Layout
Implement the simd ready layout concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Aligning data to arbitrary boundaries.** SIMD operations require specific alignment (16 bytes for SSE, 32 for AVX). Use alignas(16) on arrays that will be processed with SIMD intrinsics. Misaligned SIMD access is either slow or crashes.

## Elite Insight
Data-oriented design is SIMD-ready by nature — contiguous arrays of floats align naturally and process in batches. Mike Acton demonstrated 100x speedups by aligning position arrays for SSE4 processing on PS3.

## Systems Thinking Connection
RPG and Crawler do not need SIMD (low entity counts). Platformer could benefit but does not implement it. Your SIMD-ready layout is the most performance-advanced optimization across all four paths.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"SIMD-Ready Layout"<<endl;
    // TODO: print all simd ready layout diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "simd-ready-layout line 1", expectedOutput: "SIMD-Ready Layout", isPattern: false },
      { id: "t2", description: "simd-ready-layout line 2", expectedOutput: "LayoutReady: SIMD-safe", isPattern: false },
    ],
    hints: [
  ...
  part2: {
    title: "Build: SIMD-Ready Layout",
    type: "game_builder",
    instructions: `# Build: SIMD-Ready Layout

Add simd ready layout to the running game.

## Did It Work?
Console shows the new simd ready layout output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
// TODO: Add simd ready layout — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** SIMD-READY LAYOUT **",SCREEN_W/2-180,8,18,GOLD);
    DrawText(TextFormat("Score: %d x%d",w.score,1+w.combo_hits/10),10,38,18,WHITE);
    DrawText(TextFormat("Wave:  %d",w.wave),10,62,18,WHITE);
    DrawText(TextFormat("HP: %d/%d",w.player_hp,w.player_max_hp),10,86,18,WHITE);
    DrawText(TextFormat("Lives: %d",w.lives),10,110,18,PINK);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "simd-ready-layout line 1", expectedOutput: "SIMD-Ready Layout", isPattern: false },
      { id: "g2", description: "simd-ready-layout output", expectedOutput: "Align: enemy_x 4-byte float", isPattern: false },
      { id: "g3", description: "simd-ready-layout complete", expectedOutput: "LayoutReady: SIMD-safe", isPattern: false },
