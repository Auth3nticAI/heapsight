...
export const lessonShooter80: Lesson = {
  id: "shooter-80-milestone-perf-report",
  title: "Milestone: Performance Report",
  description: "Timing, pools, entities all reported.",
  order: 80,
  xpReward: 300,
  tier: "pro",
  concepts: ["performance report", "profiling completeness", "milestone"],
  part1: {
...
## Mental Model
Performance is not about guessing. It is about measuring. Timing, pools, entities all reported.

## What Breaks Without This
Without a performance report, optimization is anecdotal. Numbers make it real.

## The Fix: Milestone: Performance Report
Implement the milestone perf report concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Writing "performance is good" without numbers.** A performance report needs data: frame time (ms), entity count, draw calls, memory usage, pool utilization. Subjective "feels fast" is not a metric.

## Elite Insight
Professional game studios produce performance reports with every milestone build. The report includes frame budget breakdown, regression comparisons, and target hardware results. Your report follows the same standard.

## Systems Thinking Connection
RPG (L75), Crawler (L55), and Platformer (L80) produce similar reports. The metrics differ by genre, but every path must quantify performance with real numbers, not subjective impressions.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"Milestone: Performance Report"<<endl;
    // TODO: print all milestone perf report diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "milestone-perf-report line 1", expectedOutput: "Milestone: Performance Report", isPattern: false },
      { id: "t2", description: "milestone-perf-report line 2", expectedOutput: "Pattern: performance-report", isPattern: false },
    ],
    hints: [
...
Add milestone perf report to the running game.

## Did It Work?
Console shows the new milestone perf report output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** MEMORY LAYOUT REPORT **",SCREEN_W/2-180,8,18,GOLD);
...
void renderSystem(World& w){
    ...
    for(int i=0;i<MAX_ENEMIES;i++) if(w.enemy_active[i]){
        ...
        DrawRectangle((int)w.enemy_x[i]+sx,(int)w.enemy_y[i]+sy,ew,24,w.enemy_color[i]);
    }
    if(w.boss.active){
...
// TODO: Add milestone perf report — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** MILESTONE: PERF REPORT **",SCREEN_W/2-180,8,18,GOLD);
    DrawText(TextFormat("Score: %d x%d",w.score,1+w.combo_hits/10),10,38,18,WHITE);
    DrawText(TextFormat("Wave:  %d",w.wave),10,62,18,WHITE);
    DrawText(TextFormat("HP: %d/%d",w.player_hp,w.player_max_hp),10,86,18,WHITE);
    DrawText(TextFormat("Lives: %d",w.lives),10,110,18,PINK);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "milestone-perf-report line 1", expectedOutput: "Milestone: Performance Report", isPattern: false },
      { id: "g2", description: "milestone-perf-report output", expectedOutput: "PerfReport: systems profiled", isPattern: false },
      { id: "g3", description: "milestone-perf-report complete", expectedOutput: "Pattern: performance-report", isPattern: false },
