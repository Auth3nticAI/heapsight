...
export const lessonShooter86: Lesson = {
  id: "shooter-86-difficulty-config",
  title: "Difficulty Config",
  description: "Config sets game params.",
  order: 86,
  xpReward: 100,
  tier: "pro",
  concepts: ["config", "difficulty", "data-driven"],
  part1: {
...
## Mental Model
Polish transforms a functional game into a felt experience. Config sets game params.

## What Breaks Without This
Hardcoded difficulty alienates both beginners and experts.

## The Fix: Difficulty Config
Implement the difficulty config concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Storing difficulty values in code instead of a config struct.** Hardcoded values require recompilation to tune. A DifficultyConfig struct that the game reads at startup enables tuning without rebuilding.

## Elite Insight
Professional games externalize balance data — enemy HP, spawn rates, scoring — into data files. Designers iterate on balance by changing data, not code. Your config struct is the first step toward this workflow.

## Systems Thinking Connection
RPG (L87) and Crawler (L47) implement data-driven difficulty. Every path externalizes balance into data. The config format varies (struct, table, file) but the principle is identical: separate tuning from logic.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"Difficulty Config"<<endl;
    // TODO: print all difficulty config diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "difficulty-config line 1", expectedOutput: "Difficulty Config", isPattern: false },
      { id: "t2", description: "difficulty-config line 2", expectedOutput: "ConfigLoaded: 4 presets", isPattern: false },
    ],
    hints: [
...
Add difficulty config to the running game.

## Did It Work?
Console shows the new difficulty config output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
// TODO: Add difficulty config — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** DIFFICULTY CONFIG **",SCREEN_W/2-180,8,18,GOLD);
    DrawText(TextFormat("Score: %d x%d",w.score,1+w.combo_hits/10),10,38,18,WHITE);
    DrawText(TextFormat("Wave:  %d",w.wave),10,62,18,WHITE);
    DrawText(TextFormat("HP: %d/%d",w.player_hp,w.player_max_hp),10,86,18,WHITE);
    DrawText(TextFormat("Lives: %d",w.lives),10,110,18,PINK);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "difficulty-config line 1", expectedOutput: "Difficulty Config", isPattern: false },
      { id: "g2", description: "difficulty-config output", expectedOutput: "Config: easy speed=1.0 enemies=3", isPattern: false },
      { id: "g3", description: "difficulty-config complete", expectedOutput: "ConfigLoaded: 4 presets", isPattern: false },
