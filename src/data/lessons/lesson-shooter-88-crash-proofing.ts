...
export const lessonShooter88: Lesson = {
  id: "shooter-88-crash-proofing",
  title: "Crash-Proofing",
  description: "Invalid data files fall back safely.",
  order: 88,
  xpReward: 100,
  tier: "pro",
  concepts: ["crash proofing", "defensive parsing", "graceful fallback"],
  part1: {
...
## Mental Model
Polish transforms a functional game into a felt experience. Invalid data files fall back safely.

## What Breaks Without This
One corrupt data file crashes the entire game. Defensive parsing prevents this.

## The Fix: Crash-Proofing
Implement the crash proofing concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Only handling errors in the happy path.** Edge cases crash silently: empty pool on spawn, negative HP, out-of-bounds grid index. Validate at every system boundary — function entry, file load, pool access.

## Elite Insight
Defensive programming in production games means: never crash, always degrade. If the pool is full, skip the spawn. If the save is corrupt, load defaults. The game continues, and the developer sees the warning.

## Systems Thinking Connection
RPG (L88) and Crawler crash-proof the same boundaries. Every path validates the same entry points: file I/O, user input, pool access, array bounds. The crash-proofing checklist is path-universal.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"Crash-Proofing"<<endl;
    // TODO: print all crash proofing diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "crash-proofing line 1", expectedOutput: "Crash-Proofing", isPattern: false },
      { id: "t2", description: "crash-proofing line 2", expectedOutput: "DefenseParse: 3 fallbacks", isPattern: false },
    ],
    hints: [
...
Add crash proofing to the running game.

## Did It Work?
Console shows the new crash proofing output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** ASSIST MODE **",SCREEN_W/2-180,8,18,GOLD);
...
// TODO: Add crash proofing — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
void hudSystem(World& w){
    DrawText("** CRASH-PROOFING **",SCREEN_W/2-180,8,18,GOLD);
    DrawText(TextFormat("Score: %d x%d",w.score,1+w.combo_hits/10),10,38,18,WHITE);
    DrawText(TextFormat("Wave:  %d",w.wave),10,62,18,WHITE);
    DrawText(TextFormat("HP: %d/%d",w.player_hp,w.player_max_hp),10,86,18,WHITE);
    DrawText(TextFormat("Lives: %d",w.lives),10,110,18,PINK);
...
int main(){
    ...
    tests: [
      { id: "g1", description: "crash-proofing line 1", expectedOutput: "Crash-Proofing", isPattern: false },
      { id: "g2", description: "crash-proofing output", expectedOutput: "CrashProof: bounds checked", isPattern: false },
      { id: "g3", description: "crash-proofing complete", expectedOutput: "DefenseParse: 3 fallbacks", isPattern: false },
