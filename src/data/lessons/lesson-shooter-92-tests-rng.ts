...
export const lessonShooter92: Lesson = {
  id: "shooter-92-tests-rng",
  title: "Unit Tests: RNG",
  description: "Sequences verified deterministic.",
  order: 92,
  xpReward: 200,
  tier: "pro",
  concepts: ["unit tests", "RNG", "deterministic verification"],
  part1: {
    title: "Concept: Unit Tests: RNG",
    type: "concept",
    instructions: `# Unit Tests: RNG

## Mental Model
Shipping is a discipline. Every professional project passes these checks. Sequences verified deterministic.

## What Breaks Without This
Without RNG tests, determinism is a hope, not a fact.

## The Fix: Unit Tests: RNG
Implement the tests rng concept in console output, then apply it to the running game in Part 2.

## Your Task
Complete the TODOs to produce the expected console output.

## Beginner Trap
**Testing RNG without fixing the seed.** If the seed changes each run, expected output changes too. Pin the seed, generate N values, compare against a known-good sequence stored in the test.

## Elite Insight
Deterministic testing with fixed seeds is the standard for any system that uses randomness. Monte Carlo simulations, cryptographic tests, and game RNG tests all use the same approach: control the seed, control the output.

## Systems Thinking Connection
RPG (L92) and Crawler (L63) test RNG determinism the same way. Every path that implements seeded RNG must also verify it deterministically. The test pattern is identical across all paths.`,
    starterCode: `#include <iostream>
using namespace std;
int main(){
    cout<<"Unit Tests: RNG"<<endl;
    // TODO: print all tests rng diagnostics
    return 0;
}`,
...
int main(){
    ...
    tests: [
      { id: "t1", description: "tests-rng line 1", expectedOutput: "Unit Tests: RNG", isPattern: false },
      { id: "t2", description: "tests-rng line 2", expectedOutput: "TestRNG: PASS", isPattern: false },
    ],
    hints: [
  ...
  part2: {
    title: "Build: Unit Tests: RNG",
    type: "game_builder",
    instructions: `# Build: Unit Tests: RNG

Add tests rng to the running game.

## Did It Work?
Console shows the new tests rng output alongside all previous outputs. Canvas shows the game with the new infrastructure active.`,
    starterCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
// TODO: Add tests rng — see instructions`,
    solutionCode: `#include <iostream>
#include "raylib.h"
#include <cmath>
using namespace std;
...
int main(){
    ...
    tests: [
      { id: "g1", description: "tests-rng line 1", expectedOutput: "Unit Tests: RNG", isPattern: false },
      { id: "g2", description: "tests-rng output", expectedOutput: "TestRNG: seed=42 first=1013904265", isPattern: false },
      { id: "g3", description: "tests-rng complete", expectedOutput: "TestRNG: PASS", isPattern: false },
